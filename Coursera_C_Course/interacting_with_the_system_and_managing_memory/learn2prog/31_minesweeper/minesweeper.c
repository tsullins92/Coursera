...
board_t * makeBoard(int w, int h, int numMines) {
  board_t * tempBoard = malloc(sizeof(*tempBoard));
  tempBoard->width = w;
  tempBoard->height = h;
  tempBoard->totalMines = numMines;
  tempBoard->board = malloc(h*sizeof(tempBoard->board));
  for(int y=0; y<h; ++y){
    tempBoard->board[y] = malloc(w*sizeof(tempBoard->board));
    for(int x=0; x<w; ++x){
      tempBoard->board[y][x]=UNKNOWN;
    }
  }
  for(int i=0; i<numMines; ++i){
    addRandomMine(tempBoard);
  }
  return tempBoard;
}
...
int countMines(board_t * b, int x, int y) {
  ...
  for(int tempY = y-1; tempY <= y+1; ++tempY){
    for(int tempX = x-1; tempX <= x+1; ++tempX){
      if(!((tempY < 0) || (tempY > b->height-1) || (tempX < 0) || (tempX > b->width-1) || ((tempY == y) && (tempX == x)))){
	if(IS_MINE(b->board[tempY][tempX])){++count;}
      }
    }
  }
  //WRITE ME!
